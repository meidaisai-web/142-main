...
const page = () => {
    const visitorsRules = [
        ...
        {
            title: "喫煙について",
            content: <>明大祭では指定場所以外での喫煙を禁止しております。キャンパス周辺の路上や公園などにおける喫煙はご遠慮ください。<br />喫煙される方は体育館連絡ブリッジ横に設置されております喫煙所をご利用ください。詳しい場所は、キャンパスマップにてご確認いただけます。</>,
            list: (
                <List mark="※">
                    <ListItem>キャンパスマップは今後公開予定です。</ListItem>
                </List>
            ),
        },
       {
           title: "立ち入り禁止エリアについて",
           content: <>キャンパス内には安全面を考慮して立ち入りを禁止しているエリアがございます。このエリアへは立ち入らないようお願いいたします。詳しい場所は、キャンパスマップにてご確認いただけます。</>,
            list: (
                <List mark="※">
                    <ListItem>キャンパスマップは今後公開予定です。</ListItem>
                </List>
        ...
        },
        {
            ...
            content: "明大祭実行委員会の許可を得ていない団体・個人の明治大学和泉キャンパス内における活動を一切禁止しております。また、政治活動や宗教活動は禁止しております。"
        },
        {
            title: "SNS上の特定団体の誹謗中傷について",
            content: "明大祭に参加している特定の団体や個人に対するSNS上での誹謗中傷はお控えください。"
        },
        {
            title: "悪天候時の対応",
        ...
        {
            title: "休憩所",
            content: "キャンパス内には休憩所を設けております。お食事などの際にご利用ください。詳しい場所は、キャンパスマップにてご確認いただけます。",
            list: (
                <List mark="※">
                    <ListItem>キャンパスマップは今後公開予定です。</ListItem>
                </List>
        ...
        <div>
            ...
            <PageTitle>ご来場のみなさまへ</PageTitle>
                ...
                {visitorsRules.map((rule, index) => (
                    <RuleItem key={index} title={rule.title}>
                        {rule.content}
                        {rule.list}
                    </RuleItem>
                ))}
