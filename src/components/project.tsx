...
export default function Project({ title, description, image, alt, tags }: ProjectProps) {
    return (
            <div className="bg-white rounded-2xl p-4 flex flex-row md:flex-col items-center gap-6 border-7 border-accent shadow-[10px_10px_0px_0px_#3571B8] w-full">
                ...
                <div className="flex-1 md:flex-none md:w-full flex flex-col gap-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {tags.map((tag) => (
                            <span key={tag} className="bg-secondary rounded-full px-3 py-1 text-lg font-semibold">
                                {`#${tag}`}
                            </span>
                        ))}
                    </div>
