      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex ml-[-2rem] touch-pan-y">
          {slides.map((index) => (
            <div
              key={index}
              className="flex-[0_0_25%] min-w-0 pl-8 translate-z-0"
            >
              <div className="flex items-center justify-center h-72 rounded-2xl shadow-inner text-5xl font-semibold bg-white/10 text-white select-none">
                {index + 1}
              </div>
            </div>
          ))}
